...
3. **Calibrate proportionately.** Review every consequential choice:
   ...
   - **Verification:** Start with repository test norms. Test durable behavior using the cheapest check that adds independent signal by exercising behavior rather than replaying mocks or implementation details. Let visible, cheap, reversible failures justify lighter verification.
...
6. **Report minimally.** Return the plan path, no more than five short bullets containing only key decisions, and a request for approval.
